...
const DIFFICULTE_DATA = {
    ...
    coriace: { 
        tech: 9, projets: 9, succes: 9, merveilles: 12, 
        argent: 5, pop: 5, cites: 2, villages: 2,
        prod_ressource: 4,
        libelle: "🛡️ Coriace"
    },
    expert: { 
        tech: 9, projets: 9, succes: 9, merveilles: 12, 
        argent: 5, pop: 3, cites: 2, villages: 2,
        prod_ressource: 4,
        libelle: "🏆 Expert"
    }
};
...
let state = {
    ...
    setup: {
        ...
        heroCities: [],
        heroRessources: { idee: 0, pierre: 0, nourriture: 0, argent: 0 },
        playerCradle: null,
        staticRegions: [],
        staticPlacements: []
    },
    ...
    // Ressources et unités Herobotus
    heroRessources: { idee: 0, pierre: 0, nourriture: 0, argent: 0 },
    heroMilitary: {
        infantry: 0,
        cavalry: 0,
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btn-start-setup').addEventListener('click', startDetailedSetup);
    document.getElementById('btn-start-quick').addEventListener('click', startQuickGame);
    document.getElementById('btn-back-to-main').addEventListener('click', backToMainSetup);
    document.getElementById('btn-reset').addEventListener('click', resetToSetup);
    document.getElementById('btn-next').addEventListener('click', drawNextCard);
    document.getElementById('btn-loc').addEventListener('click', drawLocation);
    document.getElementById('btn-open-score').addEventListener('click', openScore);
    
    // Boutons de validation
    document.getElementById('btn-validate-hero-cities').addEventListener('click', validateHeroCities);
    document.getElementById('btn-validate-hero-ressources').addEventListener('click', validateHeroRessources);
    document.getElementById('btn-validate-statique').addEventListener('click', validateStatique);
    document.getElementById('btn-start-game').addEventListener('click', finalizeSetupAndStart);
    
    // Modal score - fermeture
    document.querySelector('.close-btn').addEventListener('click', closeScore);
    document.querySelector('.btn-modal-close').addEventListener('click', closeScore);
    
    // Gestionnaires pour les boutons de ressources du joueur
    document.querySelectorAll('.btn-res-minus, .btn-res-plus').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const isPlus = e.target.classList.contains('btn-res-plus');
            const resControl = e.target.closest('.res-control');
            if (resControl) {
                const resId = resControl.querySelector('.res-value').id;
                changeRes(resId, isPlus ? 1 : -1);
            }
        });
    });
...
function startDetailedSetup() {
    state.conseiller = document.getElementById('select-conseiller').value;
    state.difficulte = document.getElementById('select-difficulte').value;
    state.expansion = document.getElementById('select-expansion').value;
...
function startQuickGame() {
    ...
    state.setup = {
        heroCradle: regions[0],
        heroSecondCradle: regions[1],
        heroCities: [regions[0], regions[1]],
        heroRessources: { idee: 2, pierre: 2, nourriture: 2, argent: 10 },
        playerCradle: regions[2],
        staticRegions: [regions[3], regions[4]],
        staticPlacements: []
    };
...
function selectHeroSecondCradle(region) {
    ...
    document.getElementById('btn-validate-hero-cities').disabled = false;
}
...
function initStep3() {
    ...
    document.getElementById('hero-idee-start').value = state.setup.heroRessources.idee;
    document.getElementById('hero-pierre-start').value = state.setup.heroRessources.pierre;
    document.getElementById('hero-nourriture-start').value = state.setup.heroRessources.nourriture;
    
    document.getElementById('btn-validate-hero-ressources').disabled = false;
}
...
function adjustHeroRessource(type, delta) {
    ...
    document.getElementById(`hero-${type}-start`).value = newValue;
            ...
            const btn = createRegionButton(region, {
                ...
                onClick: () => {
                    ...
                    if (state.setup.staticRegions.length === 2 && 
                        ...
                        document.getElementById('btn-validate-statique').disabled = false;
                    }
                }
            });
...
function initStep6() {
    ...
    recap.innerHTML = `
        ...
        <li><i class="fas fa-coins"></i> Ressources: 💡${state.setup.heroRessources.idee} 🪨${state.setup.heroRessources.pierre} 🌾${state.setup.heroRessources.nourriture}</li>
...
function finalizeSetupAndStart() {
    state.heroRessources = { ...state.setup.heroRessources, argent: 10 };
...
function updateHeroRessourcesDisplay() {
    document.getElementById('hero-res-idee').innerText = state.heroRessources.idee;
    document.getElementById('hero-res-pierre').innerText = state.heroRessources.pierre;
    document.getElementById('hero-res-nourriture').innerText = state.heroRessources.nourriture;
    document.getElementById('hero-res-argent').innerText = state.heroRessources.argent;
}

function updateHeroMilitaryDisplay() {
    document.getElementById('hero-infantry').innerText = state.heroMilitary.infantry;
    document.getElementById('hero-cavalry').innerText = state.heroMilitary.cavalry;
    document.getElementById('hero-siege').innerText = state.heroMilitary.siege;
}

function updateHeroPilesDisplay() {
    document.getElementById('pile-tech-symbole-count').innerText = state.heroPiles.techSymbole.length;
    document.getElementById('pile-tech-fixes-count').innerText = state.heroPiles.techFixes.length;
    document.getElementById('pile-projets-count').innerText = state.heroPiles.projets.length;
    document.getElementById('pile-population-count').innerText = state.heroPiles.population.length;
}

function updateDeckCount() {
    const count = state.actionDeck.length;
    document.getElementById('deck-count').innerHTML = `<i class="fas fa-layer-group"></i> Cartes : <span>${count}</span>`;
}
...
function updatePendingDisplay() {
    const pendingText = state.pending ? 
        ...
        "-";
    document.getElementById('pending-display').innerHTML = `<i class="fas fa-hourglass-half"></i> Attente : <span>${pendingText}</span>`;
}
...
function initializeDecks() {
    // Paquet d'actions
    state.actionDeck = [...ACTIONS_BASE, ...ACTIONS_BASE];
    shuffleArray(state.actionDeck);
    
    // Paquet de lieux (régions)
    state.locationDeck = [];
    for(let i = 0; i < 4; i++) {
...
function createImplantationDeck() {
    ...
    switch(state.expansion) {
        case 'isolationniste':
            ...
            deck.push({ type: 'cradle', region: state.setup.heroCradle });
            deck.push({ type: 'cradle', region: state.setup.heroSecondCradle });
        ...
        case 'agressif':
            ...
            deck.push({ type: 'reshuffle' });
            deck.push({ type: 'penurie' });
            deck.push({ type: 'region', region: state.setup.playerCradle });
            state.setup.staticRegions.forEach(region => {
                deck.push({ type: 'region', region: region });
            });
            break;
...
function displayCard(card) {
    ...
    if (card.type === "taxe" || card.type === "pop") {
        ...
        document.getElementById('card-priority-rule').innerHTML = "⚖️ Ex-aequo = Symbole conseiller";
        ...
        document.getElementById('card-priority-rule').innerHTML = "🔄 Effet spécial du conseiller";
        ...
        contentEl.innerHTML = card.content;
        document.getElementById('card-priority-rule').innerHTML = card.footer;
        placeMilitaryUnits();
    } else {
...
function handleActionWithCost(card) {
    ...
    // Vérifier si Herobotus peut payer
    if (card.cost.pierre && state.heroRessources.pierre < card.cost.pierre) {
        canPay = false;
        missingResource = "Pierre";
        resourceIcon = "🪨";
    } else if (card.cost.idee && state.heroRessources.idee < card.cost.idee) {
        canPay = false;
        missingResource = "Idée";
        resourceIcon = "💡";
    } else if (card.cost.nourriture && state.heroRessources.nourriture < card.cost.nourriture) {
        canPay = false;
    ...
    if (canPay) {
        // Payer le coût
        if (card.cost.pierre) {
            state.heroRessources.pierre -= card.cost.pierre;
            animateRessourceChange('hero-res-pierre');
        }
        if (card.cost.idee) {
            state.heroRessources.idee -= card.cost.idee;
            animateRessourceChange('hero-res-idee');
        }
        if (card.cost.nourriture) {
            state.heroRessources.nourriture -= card.cost.nourriture;
            animateRessourceChange('hero-res-nourriture');
        }
        ...
        if (card.production) {
            if (card.production.pierre) {
                state.heroRessources.pierre += productionAmount;
                animateRessourceChange('hero-res-pierre');
            }
            if (card.production.idee) {
                state.heroRessources.idee += productionAmount;
                animateRessourceChange('hero-res-idee');
            }
            if (card.production.nourriture) {
                state.heroRessources.nourriture += productionAmount;
                animateRessourceChange('hero-res-nourriture');
            }
        }
...
function executeAction(actionType) {
    ...
    switch(actionType) {
        ...
        case 'taxe':
            // Collecter l'argent
            state.heroRessources.argent += 10;
...
function placeMilitaryUnits() {
    ...
    if (card.type === 'region') {
        region = card.region;
    } else if (card.type === 'cradle') {
        region = card.region;
    } else if (card.type === 'penurie') {
        region = state.setup.heroCities[0];
    }
    ...
    contentEl.innerHTML = `
        <div class="success-message">
            ...
            <small><i class="fas fa-map-marker-alt"></i> ${REGIONS_EMOJI[region]} ${region}</small>
        </div>
    `;
}
...
function handlePendingCard(card) {
    ...
    if (state.pending === (isTaxe ? "pop" : "taxe")) {
        ...
        if (card.cost && card.cost.nourriture && state.heroRessources.nourriture < card.cost.nourriture) {
            canPay = false;
        }
        ...
        state.pending = null;
        document.getElementById('card-priority-rule').innerHTML = "<i class='fas fa-check'></i> Action combinée réalisée";
    } else {
        ...
        if (canPay) {
            ...
            document.getElementById('card-priority-rule').innerHTML = "<i class='fas fa-hourglass-half'></i> Carte en attente";
        } else {
            // Production si pas assez de ressources
            if (card.production && card.production.nourriture) {
                state.heroRessources.nourriture += productionAmount;
...
function drawLocation() {
    ...
    if (card.type === 'region') {
        locationText = `${REGIONS_EMOJI[card.region]} ${card.region}`;
    } else if (card.type === 'penurie') {
        ...
        locationText = `🏛️ Berceau: ${REGIONS_EMOJI[card.region]} ${card.region}`;
        locationIcon = "fa-baby";
    }
    
    document.getElementById('loc-display').innerHTML = `<i class="fas ${locationIcon}"></i> Lieu : <span>${locationText}</span>`;
}

// --- GESTION DES RESSOURCES JOUEUR ---

function changeRes(id, delta) {
    const el = document.getElementById(id);
    if (!el) return;
    
    let currentValue = parseInt(el.innerText) || 0;
...
function initScoreTabs() {
    ...
    document.getElementById('btn-calculate-score').addEventListener('click', calculateAndShowResult);
    document.getElementById('btn-reset-saisie').addEventListener('click', resetAllSaisie);
    document.getElementById('btn-calculate-hero-score').addEventListener('click', calculateHeroScore);
    document.getElementById('btn-reset-hero').addEventListener('click', resetHeroSaisie);
    document.querySelector('.btn-back-to-saisie').addEventListener('click', () => switchTab('saisie'));
}
...
function updateDifficulteInfo() {
    ...
    document.getElementById('score-difficulte').innerHTML = `
        <i class="fas fa-chart-line"></i> <strong>${d.libelle}</strong> - Production: ${d.prod_ressource} ressources
    `;
    document.getElementById('difficulte-rappel').innerHTML = `
...
function resetResultatTab() {
    document.getElementById('resultat-categories').innerHTML = '';
...
function resetHeroSaisie() {
    ...
    document.getElementById('hero-score-result').style.display = 'none';
}
...
function calculateAndShowResult() {
    ...
    document.getElementById('resultat-categories').innerHTML = html;
...
function calculateHeroScore() {
    ...
    document.getElementById('hero-score-details').innerHTML = details;
    document.getElementById('hero-total-calc').innerHTML = `<i class="fas fa-calculator"></i> Score total: ${heroTotal} PV`;
    document.getElementById('hero-score-result').style.display = 'block';
}
